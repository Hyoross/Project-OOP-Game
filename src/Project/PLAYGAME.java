...
public class PLAYGAME extends JPanel implements ActionListener {
    ...
    public ENEMY spaenemy = new ENEMY();
        ...
        public void run() {
            while (true) {
                try {
                    ...
                    if (spaenemy.x > 10 && spaenemy.x < getWidth() - 1) {
                        if (enemy.x == getWidth() || enemy.x == getWidth() - 1) {
                            enemy.x = getWidth();
                            enemy.speed = 0;
                        }
                    }
        ...
        public void run() {
            while (true) {
                try {
                    ...
                    if (spaenemy.x > 10 && spaenemy.x < getWidth() - 1) {
                        if (skyenemy.x == getWidth() || skyenemy.x == getWidth() - 1) {
                            skyenemy.x = getWidth();
                            skyenemy.speed = 0;
                        }
                    }
    ...
    private int enspa = 0;
    private int spapic = 0;
    public int cout = 0;
    Thread enspcial = new Thread(new Runnable() {
        @Override
        public void run() {
            while (true) {
                try {
                    ...
                    if (spaenemy.x == getWidth()) {
                        int x = (int) (Math.random() * 10) % 10;
                        if (x >= 0) {
                            spaenemy.x = getWidth() - 1;
                            spaenemy.speed = globalspeed +2;
                        } else {
                            spaenemy.speed = 0;
                            if (score != 0) {
                                Thread.sleep(5000);
                    ...
                    else if (spaenemy.x <= 10) {
                        spaenemy.x = getWidth();
                        Thread.sleep(30000);
                    } else {
                        spaenemy.x -= spaenemy.speed;
                    }
                    if (Intersect(charac.getbound(), spaenemy.getbound())) {
                        time.stop();
                        spaenemy.x = getWidth();
                        alive = false;
                    }
    ...
    PLAYGAME() {
        ...
        this.addKeyListener(new KeyAdapter() {
            ...
            public void keyReleased(KeyEvent e) {
                super.keyReleased(e);
                if (e.getKeyCode() == KeyEvent.VK_DOWN||e.getKeyCode() == 83||e.getKeyCode() == 17) {
                    charac.run();
                    repaint();
                }
            }
    ...
    public void paintComponent(Graphics g) {
        ...
        g.setFont(new Font("Playtime", Font.CENTER_BASELINE, 50));
        g.setColor(Color.WHITE);
        g.drawString("SCORE : " + this.score, 10, 50);
        if (playpause == true) {
            g.drawImage(pause.getImage(), 920, 20, 50, 50, this);
        } else if (playpause == false) {
            g.drawImage(play.getImage(), 920, 20, 50, 50, this);
        }
        if (charac.status == 0) {
        ...
        g.drawImage(spaenemy.special[spapic].getImage(), spaenemy.x, spaenemy.y, spaenemy.width, spaenemy.height, this);
        g.drawImage(item.pic.getImage(), item.x, item.y, item.width, item.height, this);
        if (playpause == false) {
            g.drawImage(close.getImage(), 920, 80, 50, 50, this);
        }
